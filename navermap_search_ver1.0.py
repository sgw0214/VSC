...
def get_lat_lng_from_naver(place_name):
    # 1. 크롬 드라이버 실행
    options = webdriver.ChromeOptions()
    options.add_argument('--headless')  # 백그라운드 실행 (선택)
    ...
    try:
        ...
        # 5. apolloState JSON 포함된 <script> 태그 찾기
        scripts = soup.find_all('script')
        json_data = None
        for script in scripts:
            if script.string and 'window.__APOLLO_STATE__=' in script.string:
                # JSON 문자열 추출
                json_text = script.string.strip().replace('window.__APOLLO_STATE__=', '')
                json_data = json.loads(json_text)
                break

        if not json_data:
            raise Exception("❌ Apollo 상태 스크립트를 찾지 못함")

        # 6. 경도(Lng), 위도(Lat) 값 찾기
        for key in json_data:
            if key.startswith("Place:") and isinstance(json_data[key], dict):
                lat = json_data[key].get("y")
                lng = json_data[key].get("x")
                if lat and lng:
                    return lat, lng

        raise Exception("❌ 위도/경도 정보를 찾지 못함")

    finally:
        driver.quit()

# 테스트
place = "탄현마을한신6단지아파트입구"
lat, lng = get_lat_lng_from_naver(place)
print(f"📍 {place}의 위도(Lat): {lat}, 경도(Lng): {lng}")
...
#                 print("🔗 현재 iframe URL:", current_url)
#                 # 정규표현식으로 위도/경도 추출
#                 lat_lng = re.search(r"lat=([\d.]+)&lng=([\d.]+)", current_url)
#                 if lat_lng:
#                     lat = lat_lng.group(1)
#                     lng = lat_lng.group(2)
#                     print(f"📍 위도: {lat}")
#                     print(f"📍 경도: {lng}")
#                 else:
#                     print("❌ 위도/경도 정보를 URL에서 찾을 수 없습니다.")
...
#                                 store_name=key_word
#                                 road_address_fi=key_word
#                                 print(f"도로명 재시도6, 'title': '{store_name}', 'address':'{road_address_fi}', 'lat':'{geocoding(road_address_fi)[0]}','lng':'{geocoding(road_address_fi)[1]}'")
...
#             "대윤프라자","덕이동 318-9","메가MGC커피 일산한뫼초점","연세일등치과의원","링키영어 탄현점","탄현동 1551","제주진뒷고기탄현점"] #퇴근
